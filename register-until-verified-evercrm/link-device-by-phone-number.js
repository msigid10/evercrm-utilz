...
function adminLogin() {
  ...
  const payload = JSON.stringify({
    email: "akunkuuntukswagger@mailinator.com",
    password: "123123123",
  });
  const response = http.post(url, payload, params);
...
function linkDeviceByPhoneNumber(userdevicetoken, phoneNumber) {
  const url = `${host}/v1/device/whatsapp/link-by-phone-number`;
  const params = {
    headers: {
      accept: "application/json, text/plain, */*",
      "content-type": "application/json; charset=UTF-8",
      Authorization: `${userdevicetoken}`,
    },
  };
  const payload = JSON.stringify({
    countryPrefix: "62",
  ...
  const response = http.post(url, payload, params);
  console.log(`pair code to link --> ${response.json().data.pairCode}`);
}
...
export default function () {
  ...
  const the_userdevicetoken = login(the_email);
  linkDeviceByPhoneNumber(the_userdevicetoken, "85934873343");
}
